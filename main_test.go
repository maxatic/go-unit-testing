...
func Test_isPrime(t *testing.T) {
	tests := []struct {
		...
		{12, false, "12 is not a prime number because it is divisible by 2!"},
...
func Test_checkNumbers(t *testing.T) {
	tests := []struct {
		...
		{"-3", "Negative numbers are not prime, by definition!"},
		{"4", "4 is not a prime number because it is divisible by 2!"},
		{"one", "Please enter a whole number!"},
		{"q", ""},
	}
...
func Test_intro(t *testing.T) {
	...
	if !strings.Contains(string(out), "Enter a whole number") {
		t.Errorf("Incorrect intro. got %s", string(out))
	}
}
...
func Test_prompt(t *testing.T) {
	...
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatalf("Pipe creating error: %v", err)
	}
	os.Stdout = w
	...
	if err != nil {
		t.Fatalf("Pipe reading error: %v", err)
	}
	expected := "-> "
